...
function focusAvailable() {
  if (!tree.value) return
  const availableNodes = tree.value.nodes.filter(n => isUnlocked(n.id) && !isCompleted(n.id))
  
  if (availableNodes.length === 0) {
    const allNodes = tree.value.nodes
  ...
  const xs = availableNodes.map(n => n.x)
  const ys = availableNodes.map(n => n.y)
  const padding = 200
  ...
  const finalW = width < 400 ? 800 : width
  const finalH = height < 300 ? 600 : height
  const finalX = width < 400 ? minX - 200 : minX
  const finalY = height < 300 ? minY - 150 : minY

  viewBox.value = { x: finalX, y: finalY, w: finalW, h: finalH }
}
...
async function exportToPng() {
  ...
  for (let i = 0; i < clonedElements.length; i++) {
    ...
    if (clonedEl.tagName === 'text' || clonedEl.tagName === 'TEXT') {
      ...
      if (textColor && textColor !== 'rgba(0, 0, 0, 0)' && textColor !== 'rgb(0, 0, 0)') {
        clonedEl.setAttribute('fill', textColor)
      }
    }
  }
  
  clone.querySelectorAll('foreignObject').forEach(fo => fo.remove())
  clone.querySelectorAll('pattern path').forEach(p => {
  ...
  img.onload = () => {
    ...
    link.download = `${tree.value!.title || 'tree'}.png`
  ...
  <div v-if="tree" class="flex flex-col h-[calc(100vh-65px)] bg-gray-50 dark:bg-gray-900">
    <div class="p-4 bg-white dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700">
      <div class="max-w-7xl mx-auto flex items-center gap-4 flex-wrap">
        ...
        <button @click="focusAvailable" class="px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition border border-gray-300 dark:border-gray-600">
          {{ t('focusAvailable') }}
        </button>
    ...
    <div ref="canvasRef" class="flex-1 overflow-hidden relative cursor-grab active:cursor-grabbing touch-none">
      ...
      >
        ...
        <g
          ...
          :class="{ 'cursor-pointer': isUnlocked(node.id) || isCompleted(node.id) }"
